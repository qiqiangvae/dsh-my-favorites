...
function ensureStyles() {
  ...
  style.textContent = `
.mf-switcher-toast{position:fixed;left:50%;bottom:28px;transform:translateX(-50%);z-index:9999;background:var(--dsw-alias-bg-layer-1,#fff);border:1px solid var(--dsw-alias-border-l2,#d9dce1);border-radius:8px;padding:8px 14px;font-size:13px;color:var(--dsw-alias-label-primary,#1f2329);box-shadow:0 4px 16px rgba(0,0,0,.12)}.mf-sessionSettings{display:flex;flex-direction:column;gap:10px}.mf-modeGroup{display:flex;gap:14px;align-items:center;flex-wrap:wrap}.mf-modeGroup label{display:inline-flex;align-items:center;gap:5px;font-size:13px;cursor:pointer}.mf-sessionSettings .mf-countRow{display:flex;align-items:center;gap:8px}.mf-sessionSettings .mf-countRow input{width:70px}.mf-sessionSettings .mf-modeNote{color:var(--dsw-alias-label-secondary,#697586);font-size:12px}
...
function useSettings(scope) {
  ...
  return { sessions: value.sessions ?? [], urls: value.urls ?? [], mode: value.mode ?? "favorites", recentCount: clampRecentCount(value.recentCount ?? DEFAULT_RECENT) };
}
...
function FavoritesLauncher({ scope, useSessions, openSession }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mf-favorites", children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(UrlTags, { urls: value.urls }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)(SessionFolder, { sessions: value.sessions, scope, useSessions, openSession })
  ] });
}
function SidebarBelowNewSessionBridge({ scope, useSessions, openSession }) {
...
var SwitcherMachine = class {
  ...
  recentItems() {
    ...
    return ordered.slice(0, windowSize).map((id) => this.roster[id]).filter((session) => !session.blank && !session.origin).slice(0, this.recentCount).map((session) => ({ id: session.id, title: sessionTitle(session, session.id), sub: session.cwd }));
  }
  openList() {
...
function SessionSwitcherOverlay({ items, activeId, hint, onConfirm, onCancel }) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mf-switcher-overlay", onClick: onCancel, children: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)("div", { className: "mf-switcher-panel", onClick: (event) => event.stopPropagation(), children: [
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mf-switcher-head", children: active ? /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_jsx_runtime.Fragment, { children: [
      ...
      active.sub ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)("small", { children: active.sub }) : null
    ] }) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)("strong", { children: "\u6536\u85CF\u4F1A\u8BDD" }) }),
    /* @__PURE__ */ (0, import_jsx_runtime.jsx)("div", { className: "mf-switcher-list", children: visible.map((item) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)("button", { type: "button", className: "mf-switcher-item", "data-active": item.id === active?.id, "data-anchor": visible.indexOf(item) === 0, onClick: () => onConfirm(item.id), children: /* @__PURE__ */ (0, import_jsx_runtime.jsx)("span", { className: "mf-switcher-title", children: item.title }) }, item.id)) }),
...
function apply(ctx) {
  ...
  ctx.effect(() => {
    try {
      ...
      switcherMachine.start({ openSession: (id) => ctx.sessions.open(id), getList: () => sessionsList.getSnapshot(), subscribeList: (fn) => sessionsList.subscribe(fn), getSettings: () => scope.getSnapshot().value ?? { sessions: [], urls: [], mode: "favorites", recentCount: DEFAULT_RECENT }, subscribeSettings: (fn) => scope.subscribe(fn) });
    } catch (e) {
